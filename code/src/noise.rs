...
mod test {
	...
	fn one_dimension() -> Result<()> {
		for octave in 1..=8 {
			...
			image.save(format!("i-{octave}.png"))?;
		}
		Ok(())
	}

	#[test]
	fn one_dimension_float() -> Result<()> {
		for octave in 1..=8 {
			let noise = PerlinNoise1DFloat {
				seed: 7669,
				scale: 0.001,
				minimum: 0.0,
				maximum: 100.0,
			...
			image.save(format!("f-{octave}.png"))?;
		}
		Ok(())
	}
