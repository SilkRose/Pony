...
It has more functions for sorting and deduping at the same time, and extending a vector, both returning `self`, like the two above examples.

Comparing within a tuple:
```rust
use pony::traits::compare;
let mut list = vec![(3, 1), (2, 2), (1, 3)];
list.sort_by(|a, b| compare(&a.0, &b.0));
